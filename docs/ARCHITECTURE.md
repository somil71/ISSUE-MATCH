...
- **Path-traversal and command-injection defenses in repository names.** To prevent directory traversal and injection attacks during Git operations, `clone_repo` validates both segments of the target repository's name against an allowlist regex (`^[A-Za-z0-9._-]+$`), rejecting special directory names (`.`, `..`) and path escape attempts before spawning any child process.
- **Environment-based cookie security.** Cookies used for OAuth state and session management dynamically toggle their `secure` flag based on the `ENVIRONMENT` config variable. They are set to `secure=True` automatically outside of local development to prevent transit over unencrypted HTTP.
